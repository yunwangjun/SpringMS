...
public class AESUtil {
    private static final String ENCODE_RULES = "chenqi";
    ...
    public static String aesEncode(String content) {
        try {
            ...
            //2.根据encodeRules规则初始化生成密钥生成器
            //生成一个128为的随机源，根据传入的字节数组
            ...
            //7. 初始化密码器，第一个参数为加密(Encrypt_mode)或者解密(Decrypt_mode)操作，第二个为使用的KEY
            cipher.init(Cipher.ENCRYPT_MODE, key);
            //8. 获取加密内容的字节数组(这里设置为utf-8)不然内容中如果有中文和英文混合中文就会解密为乱码
            byte[] byteEncode = content.getBytes("utf-8");
            //9. 根据密码器的初始化方式一一加密，将数据加密
            byte[] byteAES = cipher.doFinal(byteEncode);
            //10. 将加密后的数据转换为字符串
            //这里用Base64Encoder中会找不到包
            //在项目的Build path中先移除JRE System Library,再添加JRE System Library,重新编译后就一切正常了
    ...
    public static String aesDecode(String content) {
        try {
             ...
             * 解密
             */
            byte[] byteDecode = cipher.doFinal(byteContent);//这里如果数组长度不对会报错
